...
if (!$isDiscovered) {
    ...
    if(!empty($data['dbHost']) )
    {
        $cfgstmp = isset($data['dbHost']['hostservercfgs'])? $data['dbHost']['hostservercfgs'] : null;
    }
...
if (!$isDiscovered) {
    ...
    $hostList->addRow(_('监测接口'), new CDiv($ifTab, 'border_dotted objectgroup inlineblock interface-group'), false, null, 'interface-row interface-row-first');
...
$app_log_level->addItem('FINEST', 'FINEST(lowest value)');
